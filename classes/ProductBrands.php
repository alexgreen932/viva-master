...
class ProductBrands {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_after_shop_loop_item_title', array( $this, 'display_brand_on_shop_page' ), 15 );
		add_action( 'init', array( $this, 'custom_woocommerce_brand_taxonomy' ) );
		add_action( 'woocommerce_single_product_summary', array( $this, 'display_brand_on_single_product_page' ), 20 );
		add_action( 'wp_head', array( $this, 'custom_brand_styles' ) );
	}
	...
	function display_brand_on_shop_page() {
		...
		if ( $terms && ! is_wp_error( $terms ) ) {
			...
			echo '<p class="product-brand-shop"> ' . implode( ', ', $brand_names ) . '</p>';
		}
	}
