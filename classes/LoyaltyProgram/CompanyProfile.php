...
class CompanyProfile {
	...
	public function __construct() {
		...
		add_action( 'show_user_profile', array( $this, 'show_custom_fields_in_profile' ) );
		add_action( 'edit_user_profile', array( $this, 'show_custom_fields_in_profile' ) );

		// Register the AJAX action
		add_action( 'wp_ajax_delete_subaccount', array( $this, 'delete_subaccount' ) );
	}

	public function register_shortcodes() {
		add_shortcode( 'sfwc_list_subaccounts', array( $this, 'list_subaccounts_shortcode' ) );
		add_shortcode( 'sfwc_edit_subaccount_form', array( $this, 'edit_subaccount_form_shortcode' ) );
	}
	...
	public function list_subaccounts_shortcode() {
		...
		$subaccounts    = get_user_meta( $parent_user_id, 'sfwc_children', true ) ?: array();
		...
		foreach ( $subaccounts as $subaccount_id ) {
			...
			if ( $subaccount ) {
				...
				echo ' <a href="' . esc_url( add_query_arg( array( 'subaccount_id' => $subaccount_id ), site_url( '/edit-subaccount' ) ) ) . '">Edit</a></li>';
			}
		}
	...
	public function delete_subaccount() {
		...
		if ( $subaccount_id && get_userdata( $subaccount_id ) ) {
			...
			if ( $deleted ) {
				wp_send_json_success( array( 'message' => 'Subaccount deleted successfully.' ) );
			} else {
				wp_send_json_error( array( 'message' => 'Failed to delete the subaccount.' ) );
			}
		} else {
			// Revoke delete_users capability if subaccount_id is invalid
			$parent_user->remove_cap( 'delete_users' );
			wp_send_json_error( array( 'message' => 'Invalid subaccount ID.' ) );
		}
	}
